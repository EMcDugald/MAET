...
#--- This script generates curls from conductivity -----------#
#--- A conductivity function is called from sigma.py       ---#
#--- and potential is obtained as a direct solution to     ---#
#--- conductivity equation.  This is repeated subject to   ---#
...
#parameters for conductivity function- this depends on the method used to generate conductivity
...
sigma_init = sig.gen_sigma_1(a,Xsm1,Xsm2,sigma_wts)
thetas = np.arange(0.,360.,360/nrots)
for theta in thetas:
    print("Rotation #: ",rot_ct, " Angle: ", theta)
    i = 1
    cond = sig.rot_sigma(theta,sigma_init)
    ...
    src_dist = config.getfloat('curr_lines',
                               'src_dist')  # defines where to terminate current line based on proximity to source#
